...
public class TriggerConditionRegister {

    private static CraftersQuestPlugin plugin = CraftersQuestPlugin.getInstance();

    public static void start() {
        Set<Class<?>> classes = getClasses("io.github.wamel04.crafters_quest.quest.trigger_condition.list");
    ...
    private static void register(Class<?> clazz) {
        try {
            ...
            TriggerCondition triggerCondition = (TriggerCondition) instance;
            TriggerCondition.triggerConditionMap.put(triggerCondition.getSymbol(), triggerCondition);
            plugin.getServer().getPluginManager().registerEvents(triggerCondition, plugin);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
